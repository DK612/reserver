<template>
    <v-card outlined>
        <v-card-title>
            Alert # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Date label="AlertDt" v-model="item.alertDt" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/alerts/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
